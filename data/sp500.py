"""
data/sp500.py
Static S&P 500 constituent list for the Alpaca (no-Polygon) scan path.

Snapshot of the index membership; it drifts by a handful of names per year.
A delisted ticker simply returns no bars and is skipped by the fetcher, and a
missing new entrant just isn't scanned - both harmless to the strategy. With
a Polygon key configured, the live constituent endpoint is used instead and
this list is ignored. Refresh occasionally, or set STOCK_UNIVERSE in .env to
override with your own comma-separated list.

Symbology is Alpaca/Polygon style (class shares use a dot: BRK.B, BF.B).
"""
...
SP500_TICKERS = [
    ...
    "TDG", "TDY", "TECH", "TEL", "TER", "TFC", "TGT", "TJX", "TMO", "TMUS",
